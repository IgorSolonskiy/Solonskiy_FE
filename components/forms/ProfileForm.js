...
export default function ProfileForm({onSubmit}) {
  ...
  const handleChangeAvatar = (event) => formik.setFieldValue("avatar",
  ...
  return (
      <form
          className="d-flex flex-column justify-content-center align-items-center mt-3 w-100"
          autoComplete="off"
        ...
        <label htmlFor="name"
               className="form-label text-center m-0  w-100 d-flex align-items-center">
          ...
          {formik.errors.name ? <div
              className="text-danger">{formik.errors.name}</div> : null}
        </label>
        <Btn name="Save" classBtn="btn btn-outline-info w-100 mt-3"
             type="submit"/>
      </form>
  );
}